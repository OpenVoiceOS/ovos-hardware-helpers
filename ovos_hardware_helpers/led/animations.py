...
class BreatheLedAnimation(LedAnimation):
    ...
    def start(self, timeout=None, one_shot=False):
        ...
        while not self.stopping.is_set():
            ...
            brightness += step
            self.leds.fill(tuple(round(brightness * part) for part in
            ...
            if one_shot and brightness >= 1:
                ...
                self.stopping.set()
        self.leds.fill((BLACK.r, BLACK.g, BLACK.b))
...
class ChaseLedAnimation(LedAnimation):
    ...
    def start(self, timeout=None, one_shot=False):
        ...
        self.leds.fill((self.background_color.r, self.background_color.g, self.background_color.b))
        while not self.stopping.is_set():
            for led in range(0, self.leds.num_leds):
                self.leds.set_led(led, (self.foreground_color.r, self.foreground_color.g, self.foreground_color.b))
                self._delay.wait(self.step_delay)
                self.leds.set_led(led, (self.background_color.r, self.background_color.g, self.background_color.b))
            if one_shot:
                self.stopping.set()
            elif end_time and time() > end_time:
                self.stopping.set()
...
class FillLedAnimation(LedAnimation):
    ...
    def start(self, timeout=None, one_shot=True):
        ...
        if self.reverse:
            leds.reverse()
        for led in leds:
            self.leds.set_led(led, (self.fill_color.r, self.fill_color.g, self.fill_color.b))
            self._delay.wait(self.step_delay)
    ...
    def start(self, timeout=None, one_shot=False):
        ...
        while not self.stopping.is_set():
            for i in range(self.num_blinks):
                self.leds.fill((self.color.r, self.color.g, self.color.b))
                self._delay.wait(0.25)
                self.leds.fill((BLACK.r, BLACK.g, BLACK.b))
                self._delay.wait(0.25)
            if one_shot:
        ...
        while not self.stopping.is_set():
            for led in range(self.leds.num_leds):
                if evens and led % 2 == 0:
                    self.leds.set_led(led, (self.color.r, self.color.g, self.color.b), False)
                elif not evens and led % 2 == 1:
                    self.leds.set_led(led, (self.color.r, self.color.g, self.color.b), False)
                else:
                    self.leds.set_led(led, (BLACK.r, BLACK.g, BLACK.b), False)
            self.leds.show()
